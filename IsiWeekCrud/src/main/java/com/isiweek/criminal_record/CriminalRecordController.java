...
public class CriminalRecordController {
    ...
    @GetMapping("/add")
    public String add(@ModelAttribute("criminalRecord") final CriminalRecord criminalRecor) {
        return "criminalRecord/add";
    }

    @PostMapping("/add")
    public String add(
            @ModelAttribute("criminalRecord") @Valid final CriminalRecord criminalRecord,
            final BindingResult bindingResult, final RedirectAttributes redirectAttributes) {
        if (!bindingResult.hasFieldErrors("name") && criminalRecordService.nameExists(criminalRecord.getName())) {
            bindingResult.rejectValue("name", "Exists.criminalRecord.name");
        }
        if (bindingResult.hasErrors()) {
            return "criminalRecord/add";
        }
        criminalRecordService.create(criminalRecord);
    ...
    public String edit(@PathVariable(name = "id") final Long id,
            @ModelAttribute("criminalRecord") @Valid final CriminalRecord criminalRecord,
            final BindingResult bindingResult, final RedirectAttributes redirectAttributes) {
        final Optional<CriminalRecord> currentCriminalRecord = criminalRecordService.get(id);
        if (!bindingResult.hasFieldErrors("name")
                && !criminalRecord.getName().equalsIgnoreCase(currentCriminalRecord.get().getName())
                && criminalRecordService.nameExists(criminalRecord.getName())) {
            bindingResult.rejectValue("name", "Exists.criminalRecord.name");
        }
        if (bindingResult.hasErrors()) {
            return "criminalRecord/edit";
        }
        criminalRecordService.update(id, criminalRecord);
    ...
    public String delete(@PathVariable(name = "id") final Long id,
        ...
        final String referencedWarning = criminalRecordService.getReferencedWarning(id);
