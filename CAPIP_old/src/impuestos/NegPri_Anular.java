...
public class NegPri_Anular extends javax.swing.JFrame {
     ...
     * @param _parent
     */
    public NegPri_Anular(final java.awt.Window _parent) {
        super();
        initComponents();
    ...
    private void actAnular() {
        ...
        try {
            ...
            final ResultSet rsRet = Conn.executeQuery("SELECT * FROM neg_pri WHERE id_neg_pri= " + id);
            if (rsRet.next()) {
                ...
                final ResultSet rsRetDet = Conn.executeQuery("SELECT * FROM neg_pri_det WHERE id_neg_pri= " + regRet.getId_neg_pri());
                while (rsRetDet.next()) {
                ...
                if (JOptionPane.showConfirmDialog(this, "Esta operación es irreversible. Desea anular la Retención ?",
                    ...
                    try {
                        ...
                        final PreparedStatement pst = Conn.getConnection().prepareStatement("UPDATE neg_pri SET anulado_sn= 'S' WHERE id_imp_mun= " + id);
                        if (pst.executeUpdate() != 1) {
                            throw new Exception("Error al tratar de actualizar el Registro");
                        }

                        // Actualizar los detalles
                        final PreparedStatement pstDet = Conn.getConnection().prepareStatement("UPDATE neg_pri_det SET anulado_sn= 'S' WHERE id_imp_mun= " + id);
                        if (pstDet.executeUpdate() <= 0) {
                            throw new Exception("Error al tratar de actualizar el Registro");
                        }

                        // Actualizar todos los Causado asociados
                        final ResultSet rsRetDet2 = Conn.executeQuery("SELECT * FROM neg_pri_det WHERE id_neg_pri= " + id);
                        while (rsRetDet2.next()) {
                            ...
                            final PreparedStatement pstCau = Conn.getConnection().prepareStatement("UPDATE causado SET neg_pri_ret_sn= 'N' WHERE id_causado= " + id_causado);
                            ...
                            while (rsCompr.next()) {
                                ...
                                final PreparedStatement pstCompr2 = Conn.getConnection().prepareStatement("UPDATE " + regCau.getTipo_compr().getTbl() + " SET neg_pri_grav_bs= 0, neg_pri_bs= 0 WHERE id_compr= ?");
