...
public class Migrar {
    ...
    public static void main(String[] args) {
        ...
        try (final ResultSet rsPag = Conn.executeQuery("SELECT * FROM orden_pago ORDER BY id_orden_pago ASC")) {
            ...
            while (rsPag.next()) {
                ...
                try (final ResultSet rsBco = Conn.executeQuery("SELECT * FROM bancos WHERE cuenta= '" + regPag.getCuenta() + "'")) {
                    if (rsBco.next()) {
                ...
                final PreparedStatement pst = Conn.getConnection().prepareStatement("UPDATE orden_pago "
                    + "SET id_cuenta= ?, num_x_pag= ?, num_x_cuenta= ? "
                    + "WHERE id_orden_pago = ?");
