...
public final class FondoAvanceEfectivoReport extends XJFrameJTable {
     ...
     * @param _parent
     */
    public FondoAvanceEfectivoReport(final java.awt.Window _parent) {
        super(_parent);
        initComponents();
    ...
    private void initComponents() {
        ...
        txtExcel.setFont(new java.awt.Font("Arial", 3, 14)); // NOI18N
    ...
    private void btnReportSummaryActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnReportSummaryActionPerformed
        ...
        try {
            Conn.getConnection().prepareStatement("DELETE FROM ava_efe_rpt_summary WHERE fchsession <= '" + fchDelete + "' OR iduser=" + UserPassIn.getIdUser() + " AND idsession=" + UserPassIn.getIdSession()).executeUpdate();
        } catch (final Exception _ex) {
            JOptionPane.showMessageDialog(null, _ex);
            return;
        }
        ...
        for (int i = 0; i < tblMaster.getRowCount(); i++) {
            ...
            try {
                final PreparedStatement pst = Conn.getConnection().prepareStatement("INSERT INTO ava_efe_rpt_summary("
    ...
    private void PagoRpt(int _selRow) {

        // recuperar el pago de la tabla
        final AvanceEfectivoModel reg = (AvanceEfectivoModel) tblMaster.getValueAt(_selRow, colReg);
        ...
        try {
            ...
            final ResultSet rs = Conn.executeQuery("SELECT * FROM avance_efectivo WHERE id='" + numPag + "'");
            if (rs.next()) {
            ...
            JOptionPane.showMessageDialog(null, "Error al tratar de recuperar la Orden de Avance: " + System.getProperty("line.separator") + _ex);
            return;
        }
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "Error al formatear el total: " + System.getProperty("line.separator") + _ex);
            return;
        }
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "Error al borrar la tabla Aux.: " + System.getProperty("line.separator") + _ex);
            return;
        }

        try {
            final PreparedStatement pst = Conn.getConnection().prepareStatement("INSERT INTO ava_efe_aux_report(codpresu,partida,total,fchsession,idsession,iduser) VALUES (?,?,?,?,?,?)");
            ...
            JOptionPane.showMessageDialog(null, "Error al insertar en la tabla Aux. Rep.: " + System.getProperty("line.separator") + _ex);
            return;
        }
     ...
     */
    @SuppressWarnings("unchecked")
    public void comprRptSingle(ParamRptPagAva _paramRpt) {
        if (_paramRpt == null) {
            return;
        }

        final Map<String, Object> param = new HashMap<>(101);
        param.put("numPag", _paramRpt.getId_orden_pago());

        final String Numliteral_1 = _paramRpt.getEnLetras_1();
        final String Numliteral_2 = _paramRpt.getEnLetras_2();

        param.put("Numliteral_1", Numliteral_1);
        param.put("Numliteral_2", Numliteral_2);

        param.put("montoPagar", _paramRpt.getMontoPagar());
        param.put("montoTotal", _paramRpt.getMontoTotal());
        param.put("resta", _paramRpt.getResta());
        param.put("razonSocial", _paramRpt.getBenef_RazonSocial());
        param.put("rif", _paramRpt.getBenef_rif_ci());
        param.put("banco", _paramRpt.getBanco());
        param.put("cuenta", _paramRpt.getCuenta());
        param.put("cheque", _paramRpt.getCheque());
        param.put("ivaRet", _paramRpt.getIvaRet());
        param.put("ivaTotal", _paramRpt.getIvaTot());
        param.put("concepto", _paramRpt.getConcepto());
        param.put("fechaHoy", _paramRpt.getFechaHoy());
        param.put("numcaus", _paramRpt.getNumCaus());
        param.put("numcomprs", _paramRpt.getNumComprs());
