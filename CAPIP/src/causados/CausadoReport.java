...
public final class CausadoReport extends XJFrameJTable {
     ...
     * @param _parent
     */
    public CausadoReport(final java.awt.Window _parent) {
        super(_parent);
        initComponents();
    ...
    private void btnReportSummaryActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnReportSummaryActionPerformed
        ...
        try {
            Conn.getConnection().prepareStatement("DELETE FROM cau_rpt_summary WHERE fchsession <= '" + fchDelete + "' OR iduser=" + UserPassIn.getIdUser() + " AND idsession=" + UserPassIn.getIdSession()).executeUpdate();

//            Conn.getConnection().prepareStatement("DELETE FROM cau_rpt_summary WHERE iduser=" + UserPassIn.getIdUser() + " AND fchsession <= '" + fchDelete + "'").executeUpdate();
        } catch (final Exception _ex) {
            JOptionPane.showMessageDialog(null, _ex);
            return;
        }
        ...
        for (int i = 0; i < tblMaster.getRowCount(); i++) {
            ...
            try {
                final PreparedStatement pst = Conn.getConnection().prepareStatement("INSERT INTO cau_rpt_summary("
