...
public class OrdPagNextNum {
     ...
     * @throws Exception
     */
    public long checkNum(CuentaModel _regCuenta) throws Exception {
        setClave_xCuenta(_regCuenta);
        id_cuenta = 0;
        num_xPag = 0;
        num_xCuenta = 0;
     ...
     * @throws Exception
     */
    public void nextNum(CuentaModel _regCuenta) throws Exception {

        num_xPag = 0;
        num_xCuenta = 0;

        if (_regCuenta != null) {
            id_cuenta = _regCuenta.getId_cuenta();
            clave_xCuenta = String.valueOf(Globales.getEjeFisYear()) + "-PAG-" + _regCuenta.getCuenta();
        } else {
            id_cuenta = 0;
        ...
        do {
            try (final PreparedStatement pstAbs = Conn.getConnection().prepareStatement("SELECT * FROM map_next_id WHERE clave = ? FOR UPDATE")) {
                pstAbs.setString(1, clave_xPag);

                try (final ResultSet rsAbs = pstAbs.executeQuery()) {
                    if (rsAbs.next()) {
                        do {
                            try (final PreparedStatement pstRel = Conn.getConnection().prepareStatement("SELECT * FROM map_next_id WHERE clave = ? FOR UPDATE")) {
                                ...
                                try (final ResultSet rsRel = pstRel.executeQuery()) {
                                    if (rsRel.next()) {
                                        ...
                                        try (final PreparedStatement pstNew = Conn.getConnection().prepareStatement("INSERT INTO map_next_id"
    ...
    public final void update() throws Exception {
        ...
        final PreparedStatement pstAbs = Conn.getConnection().prepareStatement("UPDATE map_next_id set valor= ? WHERE clave= ?");
        ...
        if ((num_xCuentaAux > 0) && (clave_xCuentaAux != null)) {
            final PreparedStatement pstRel = Conn.getConnection().prepareStatement("UPDATE map_next_id set valor= ? WHERE clave= ?");
     ...
     * @param _regCuenta
     */
    public void setClave_xCuenta(CuentaModel _regCuenta) {
        if (_regCuenta == null) {
            clave_xCuenta = String.valueOf(Globales.getEjeFisYear()) + "-PAG-" + "00000000000000000000";
        } else {
            clave_xCuenta = String.valueOf(Globales.getEjeFisYear()) + "-PAG-" + _regCuenta.getCuenta();
