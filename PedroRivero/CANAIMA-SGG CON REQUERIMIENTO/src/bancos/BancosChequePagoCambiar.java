...
public final class BancosChequePagoCambiar extends javax.swing.JFrame {
     ...
     */
    public BancosChequePagoCambiar(final java.awt.Window _parent, final long _id_orden_pago) {
        super();
        initComponents();

        parent = _parent;
        id_orden_pago = _id_orden_pago;
    ...
    private void setStartConditions() {
        ...
        try {
            final ResultSet rs = Conn.executeQuery("SELECT * FROM orden_pago WHERE id_orden_pago= " + id_orden_pago);
            if (rs.next()) {
    ...
    private void actGuardar() {
        ...
        try {
            ...
            final ResultSet rsPag = Conn.executeQuery("SELECT * FROM orden_pago WHERE id_orden_pago= " + id_orden_pago + " FOR UPDATE");
            if (rsPag.next()) {
                final PagoModel regPag = new PagoModel(rsPag);

                final PreparedStatement psOrdPag = Conn.getConnection().prepareStatement("UPDATE orden_pago set cheque= ? WHERE id_orden_pago= ?");
                ...
                final PreparedStatement psBanDet = Conn.getConnection().prepareStatement("UPDATE bancos_operaciones set soporte_o_cheque= ? WHERE (tipo_operacion= 'CH' OR tipo_operacion= 'SC') AND num_pag_ava= ? AND anulado_sn= 'N'");
                ...
                final PreparedStatement pst = Conn.getConnection().prepareStatement("INSERT INTO bancos_ch_anu"
            ...
            JOptionPane.showMessageDialog(null, "Error al tratar de realizar los cambios" + System.getProperty("line.separator") + _ex);
        }

    }

    @SuppressWarnings({"unchecked"})
    private void rptPlanillaAnuPag(final PagoModel _regPA, final BancosChAnuModel _regAnu) throws Exception {
