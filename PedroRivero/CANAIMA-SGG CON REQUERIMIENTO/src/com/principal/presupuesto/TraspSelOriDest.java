...
public final class TraspSelOriDest extends XJFrameJTable {
     ...
     * @param inparent
     * @param instrTbl
     * @param instrTipoOriDest
     */
    public TraspSelOriDest(final java.awt.Window inparent, final String instrTbl, final String instrTipoOriDest) {
        super(inparent);
        strTbl = instrTbl;
        strTipoOriDest = instrTipoOriDest;
        initComponents();
        setOwnBehavior();
        setCompBehavior();
        setStartConditions();
        ...
        try {
            ...
            if (regTraspDet == null) {
                if (montoTrasp.compareTo(ZERO) > 0) {
                    final PreparedStatement ps = ConnPpto.conn.prepareStatement(// 1, 2, 3, 4,
                            "INSERT INTO trasppartidas_det "
                            + // 5, 6, 7, 8, 9
                            "(id_user, id_session, date_session, id_trasp, " + "ppto_egr_ing, tipo_ori_dest, codpresu, partida, monto) " + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
                    ...
                    ps.setLong(4, 0);
                    // ppto_egr_ing
                    ps.setString(5, strTbl.equals(Globales.TBL_PPTO_EGRESO) ? "E" : "I");
                    // tipo_ori_dest
                    ps.setString(6, strTipoOriDest);
                    // codpresu
                    ps.setString(7, regPpto.getCodigo());
                    // partida
                    ps.setString(8, regPpto.getPartida());
                    // monyo
                    ps.setBigDecimal(9, montoTrasp);
                    ps.executeUpdate();
                }
            } else {
                if (montoTrasp.compareTo(ZERO) > 0) {
    ...
    void initComponents() {
        ...
        tblMaster.setModel(new javax.swing.table.DefaultTableModel(new Object[][] {}, new String[] { "Código", "Partida", "Monto Ini. Bs.", "Monto Disp. Bs.", "Monto Trasp. Bs.", "RegPpto", "RegTraspDet" }) {
...
//GEN-FIRST:event_btnRetornarActionPerformed
        // Codigo programado
    }
...
//GEN-LAST:event_btnReIniciarActionPerformed
    ...
    private static final Logger logger = LogManager.getLogger(TraspSelOriDest.class);
}
