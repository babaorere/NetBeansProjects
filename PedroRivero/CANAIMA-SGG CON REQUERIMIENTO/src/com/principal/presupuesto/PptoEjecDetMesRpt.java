...
import com.principal.modelos.PptoEjecDetModel;
import com.principal.modelos.PptoEjecDetRptModel;
...
import com.principal.utils.EjecMov;
...
public final class PptoEjecDetMesRpt extends XJFrameJTable {
     ...
     * @param inpartida
     */
    public PptoEjecDetMesRpt(final java.awt.Window inparent, final PptoModel inpartida) {
        super(inparent);
        initComponents();
        partida = inpartida;
        if (partida == null) {
    ...
    private ArrayList<PptoEjecDetModel> genList() {
        ...
        try {
            ...
            for (TipoCompr tablaCompr : TipoCompr.values()) {
                ...
                try (final PreparedStatement pstCompr = ConnCapip.getInstance().getConnection().prepareStatement("SELECT * FROM " + tablaCompr.getTbl() + " WHERE (ejefis >= '" + strMesDesde + "') AND (ejefis < '" + strMesHastaEx + "') AND anulado_sn= 'N' ORDER BY fecha_compr")) {
                    try (final ResultSet rsCompr = pstCompr.executeQuery()) {
                        while (rsCompr.next()) {
                            ...
                            try (final PreparedStatement pstComprDet = ConnCapip.getInstance().getConnection().prepareStatement("SELECT * FROM compr_det WHERE id_compr= ? AND tipo_compr= ? AND codpresu= ?")) {
            ...
            try (final PreparedStatement pstCre = ConnCapip.getInstance().getConnection().prepareStatement("SELECT * FROM creditoadicional WHERE (ejefismes >= '" + mesDesdeIn + "') AND (ejefismes < '" + mesHastaEx + "') AND anulado_sn= 'N' ORDER BY fecha")) {
                try (final ResultSet rsCre = pstCre.executeQuery()) {
                    while (rsCre.next()) {
                        ...
                        try (final PreparedStatement pstCreDet = ConnCapip.getInstance().getConnection().prepareStatement("SELECT * FROM creditoadicional_det WHERE id_cre_adi= ? AND codigo= ?")) {
            ...
            try (final PreparedStatement pstTra = ConnCapip.getInstance().getConnection().prepareStatement("SELECT * FROM trasppartidas WHERE (ejefismes >= '" + mesDesdeIn + "') AND (ejefismes < '" + mesHastaEx + "') AND anulado_sn= 'N' AND ppto_egr_ing= 'E' ORDER BY fecha")) {
                try (final ResultSet rsTrasp = pstTra.executeQuery()) {
                    while (rsTrasp.next()) {
                        ...
                        try (final PreparedStatement pstTraspDet = ConnCapip.getInstance().getConnection().prepareStatement("SELECT * FROM trasppartidas_det WHERE id_trasp= ? AND codpresu= ? AND ppto_egr_ing= 'E'")) {
...
//GEN-LAST:event_btnSeleccionarActionPerformed
    ...
    private static final Logger logger = LogManager.getLogger(PptoEjecDetMesRpt.class);
}
