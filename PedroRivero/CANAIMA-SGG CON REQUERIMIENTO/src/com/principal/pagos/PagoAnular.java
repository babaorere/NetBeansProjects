...
public class PagoAnular extends javax.swing.JFrame {
    ...
    void initComponents() {
        ...
        // NOI18N
        chkMostrarReporte.setFont(new java.awt.Font("Tahoma", 0, 14));
        ...
        // NOI18N
        lblCuenta.setFont(new java.awt.Font("Tahoma", 0, 14));
                    ...
                    try {
                        ...
                        try (final PreparedStatement pstBancos = ConnCapip.getInstance().getConnection().prepareStatement("UPDATE bancos_operaciones SET anulado_sn= 'S' " + "WHERE anulado_sn = 'N' AND (tipo_operacion = 'ND' OR tipo_operacion = 'CH') " + "AND num_pag_ava= ?")) {
                            ...
                            if (pstBancos.executeUpdate() > 1) {
                                // Hay ordenes sin operacion en Banco, pago directo
                                    ...
                                    if (regCau != null) {
                                        ...
                                        try (final PreparedStatement pstCompr = ConnCapip.getInstance().getConnection().prepareStatement("SELECT * FROM compr_otros WHERE id_causado= ?")) {
                                            ...
                                            try (final ResultSet rsCompr = pstCompr.executeQuery()) {
                                                while (rsCompr.next()) {
                                                    // false para no generar el reporte, que por alguna razon genera error
                                                    actAnularCompr(rsCompr.getLong("id_compr"), regCau.getTipo_compr(), false);
                                                }
                                            }
                                        }
                                        // Anular Causado
...
//GEN-LAST:event_txtNumKeyTyped
    ...
    private static final Logger logger = LogManager.getLogger(PagoAnular.class);
}
