...
public class ISLR_Anular extends javax.swing.JFrame {
     ...
     * @param inparent
     */
    public ISLR_Anular(final java.awt.Window inparent) {
        super();
        initComponents();
        parent = inparent;
        setOwnBehavior();
    ...
    void initComponents() {
        ...
        // NOI18N
        btnAceptar.setFont(new java.awt.Font("Arial", 2, 18));
...
//GEN-LAST:event_btnAceptarActionPerformed
    ...
    private void actAnular() {
        ...
        try {
            ...
            final ResultSet rsRet = ConnCapip.getInstance().executeQuery("SELECT * FROM islr_retencion WHERE id_islr_retencion= " + id);
            if (rsRet.next()) {
                ...
                final ResultSet rsRetDet = ConnCapip.getInstance().executeQuery("SELECT * FROM islr_retencion_det WHERE id_islr_retencion= " + regRet.getId_islr_retencion());
                while (rsRetDet.next()) {
                ...
                if (JOptionPane.showConfirmDialog(this, "Esta operación es irreversible. Desea anular la Retención ?", "Confirmar acción", JOptionPane.YES_NO_OPTION) == JOptionPane.OK_OPTION) {
                    try {
                        ...
                        final PreparedStatement pst = ConnCapip.getInstance().getConnection().prepareStatement("UPDATE islr_retencion SET anulado_sn= 'S' WHERE id_islr_retencion= " + id);
                        if (pst.executeUpdate() != 1) {
                            throw new Exception("Error al tratar de actualizar el Registro");
                        }
                        // Actualizar los detalles
                        final PreparedStatement pstDet = ConnCapip.getInstance().getConnection().prepareStatement("UPDATE islr_retencion_det SET anulado_sn= 'S' WHERE id_islr_retencion= " + id);
                        if (pstDet.executeUpdate() <= 0) {
                            throw new Exception("Error al tratar de actualizar el Registro");
                        }
                        // Actualizar todos los Causado asociados
                        final ResultSet rsRetDet2 = ConnCapip.getInstance().executeQuery("SELECT * FROM islr_retencion_det WHERE id_islr_retencion= " + id);
                        while (rsRetDet2.next()) {
                            ...
                            final PreparedStatement pstCau = ConnCapip.getInstance().getConnection().prepareStatement("UPDATE causado SET islr_ret_sn= 'N' WHERE id_causado= " + regRetDet.getId_causado());
                            ...
                            while (rsCompr.next()) {
                                ...
                                final PreparedStatement pstCompr2 = ConnCapip.getInstance().getConnection().prepareStatement("UPDATE " + regCau.getTipo_compr().getTbl() + " SET islr_grav_bs= 0, islr_porc_ret= 0 WHERE id_compr= ?");
...
//GEN-LAST:event_txtNumKeyTyped
    ...
    private static final Logger logger = LogManager.getLogger(ISLR_Anular.class);
}
