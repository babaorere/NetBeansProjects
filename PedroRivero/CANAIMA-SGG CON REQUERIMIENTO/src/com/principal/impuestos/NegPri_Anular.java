 ...
 */
public class NegPri_Anular extends javax.swing.JFrame {
     ...
     * @param inparent
     */
    public NegPri_Anular(final java.awt.Window inparent) {
        super();
        initComponents();
        parent = inparent;
        setOwnBehavior();
    ...
    void initComponents() {
        ...
        setTitle("ANULACIÓN DE NEGRO PRIMERO");
...
//GEN-LAST:event_btnAceptarActionPerformed
    ...
    private void actAnular() {
        ...
        try {
            ...
            final ResultSet rsRet = ConnCapip.getInstance().executeQuery("SELECT * FROM neg_pri WHERE id_neg_pri= " + id);
            if (rsRet.next()) {
                final NegPriModel regRet = new NegPriModel(rsRet);
                ...
                final ResultSet rsRetDet = ConnCapip.getInstance().executeQuery("SELECT * FROM neg_pri_det WHERE id_neg_pri= " + regRet.getId_neg_pri());
                while (rsRetDet.next()) {
                    final NegPri_DetModel regRetDet = new NegPri_DetModel(rsRetDet);
                ...
                if (JOptionPane.showConfirmDialog(this, "Esta operación es irreversible. Desea anular la Retención ?", "Confirmar acción", JOptionPane.YES_NO_OPTION) == JOptionPane.OK_OPTION) {
                    try {
                        ...
                        final PreparedStatement pst = ConnCapip.getInstance().getConnection().prepareStatement("UPDATE neg_pri SET anulado_sn= 'S' WHERE id_imp_mun= " + id);
                        if (pst.executeUpdate() != 1) {
                            throw new Exception("Error al tratar de actualizar el Registro");
                        }
                        // Actualizar los detalles
                        final PreparedStatement pstDet = ConnCapip.getInstance().getConnection().prepareStatement("UPDATE neg_pri_det SET anulado_sn= 'S' WHERE id_imp_mun= " + id);
                        if (pstDet.executeUpdate() <= 0) {
                            throw new Exception("Error al tratar de actualizar el Registro");
                        }
                        // Actualizar todos los Causado asociados
                        final ResultSet rsRetDet2 = ConnCapip.getInstance().executeQuery("SELECT * FROM neg_pri_det WHERE id_neg_pri= " + id);
                        while (rsRetDet2.next()) {
                            final NegPri_DetModel regRetDet = new NegPri_DetModel(rsRetDet2);
                            ...
                            final PreparedStatement pstCau = ConnCapip.getInstance().getConnection().prepareStatement("UPDATE causado SET neg_pri_ret_sn= 'N' WHERE id_causado= " + id_causado);
                            if (pstCau.executeUpdate() != 1) {
                                throw new Exception("Error al tratar de actualizar el Registro");
                            }
                            // Buscar todos lo compromisos asociados al causado
                            final ResultSet rsCompr = ConnCapip.getInstance().executeQuery("SELECT * FROM " + regCau.getTipo_compr().getTbl() + " WHERE id_causado= " + id_causado);
                            while (rsCompr.next()) {
                                final ComprModel regCompr = new ComprModel(rsCompr);
                                final PreparedStatement pstCompr2 = ConnCapip.getInstance().getConnection().prepareStatement("UPDATE " + regCau.getTipo_compr().getTbl() + " SET neg_pri_grav_bs= 0, neg_pri_bs= 0 WHERE id_compr= ?");
                                ...
                                final PreparedStatement pstCau2 = ConnCapip.getInstance().getConnection().prepareStatement("UPDATE causado SET resta_bs= resta_bs + ? WHERE id_causado= ?");
                                pstCau2.setBigDecimal(1, regCompr.getNeg_pri_bs());
                    ...
                    try {
                        ...
                        ImpuestoRetencion.genNegPriReport(id, param);
                    } catch (final Exception inex) {
...
//GEN-LAST:event_txtNumKeyTyped
    ...
    public static void main(String[] args) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            ...
            @Override
            public void run() {
                new NegPri_Anular(null).setVisible(true);
            }
        });
    }
    ...
    private static final Logger logger = LogManager.getLogger(NegPri_Anular.class);
}
