...
import com.principal.modelos.NegPriRptModel;
...
public final class NegPri_Rend extends javax.swing.JFrame {
     ...
     * @param inparent
     */
    public NegPri_Rend(final java.awt.Window inparent) {
        super();
        initComponents();
        parent = inparent;
        setOwnBehavior();
    ...
    void initComponents() {
        ...
        getContentPane().add(jScrollPane1, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 50, 1040, 410));
        ...
        getContentPane().add(jLabel7, new org.netbeans.lib.awtextra.AbsoluteConstraints(-10, -10, 1070, 600));
        pack();
    ...
    private void ReporteRendicion() {
        ...
        for (int i = 0; i < tblMaster.getRowCount(); i++) {
            ...
            list.add(new NegPriRptModel(new java.util.Date(reg.getEjefis().getTime()), reg.getAnulado_sn().equals("S") ? "S" : "", reg.getId_neg_pri(), reg.getNum_fact(), reg.getBenef_rif_ci(), reg.getBenef_razonsocial(), reg.getTotal_fact(), reg.getBase_imponible(), reg.getRetenido_bs()));
    ...
    private static final Logger logger = LogManager.getLogger(NegPri_Rend.class);
}
