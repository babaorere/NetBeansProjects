...
public class ImpMun_Anular extends javax.swing.JFrame {
     ...
     * @param inparent
     */
    public ImpMun_Anular(final java.awt.Window inparent) {
        super();
        initComponents();
        parent = inparent;
        setOwnBehavior();
...
//GEN-LAST:event_btnAceptarActionPerformed
    ...
    private void actAnular() {
        ...
        try {
            ...
            final ResultSet rsRet = ConnCapip.getInstance().executeQuery("SELECT * FROM imp_mun WHERE id_imp_mun= " + id);
            if (rsRet.next()) {
                ...
                final ResultSet rsRetDet = ConnCapip.getInstance().executeQuery("SELECT * FROM imp_mun_det WHERE id_imp_mun= " + regRet.getId_imp_mun());
                while (rsRetDet.next()) {
                ...
                if (JOptionPane.showConfirmDialog(this, "Esta operación es irreversible. Desea anular la Retención ?", "Confirmar acción", JOptionPane.YES_NO_OPTION) == JOptionPane.OK_OPTION) {
                    try {
                        ...
                        final PreparedStatement pst = ConnCapip.getInstance().getConnection().prepareStatement("UPDATE imp_mun SET anulado_sn= 'S' WHERE id_imp_mun= " + id);
                        if (pst.executeUpdate() != 1) {
                            throw new Exception("Error al tratar de actualizar el Registro");
                        }
                        // Actualizar los detalles
                        final PreparedStatement pstDet = ConnCapip.getInstance().getConnection().prepareStatement("UPDATE imp_mun_det SET anulado_sn= 'S' WHERE id_imp_mun= " + id);
                        if (pstDet.executeUpdate() <= 0) {
                            throw new Exception("Error al tratar de actualizar el Registro");
                        }
                        // Actualizar todos los Causado asociados
                        final ResultSet rsRetDet2 = ConnCapip.getInstance().executeQuery("SELECT * FROM imp_mun_det WHERE id_imp_mun= " + id);
                        while (rsRetDet2.next()) {
                            ...
                            final PreparedStatement pstCau = ConnCapip.getInstance().getConnection().prepareStatement("UPDATE causado SET imp_mun_ret_sn= 'N' WHERE id_causado= " + id_causado);
                            ...
                            while (rsCompr.next()) {
                                ...
                                final PreparedStatement pstCompr2 = ConnCapip.getInstance().getConnection().prepareStatement("UPDATE " + regCau.getTipo_compr().getTbl() + " SET imp_mun_grav_bs= 0, imp_mun_bs= 0 WHERE id_compr= ?");
...
//GEN-LAST:event_txtNumKeyTyped
    ...
    private static final Logger logger = LogManager.getLogger(ImpMun_Anular.class);
}
