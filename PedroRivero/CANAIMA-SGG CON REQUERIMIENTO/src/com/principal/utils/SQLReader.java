...
public class SQLReader {
     ...
     */
    public static ArrayList<String> createQueries(final InputStream ininSt) {
        String queryLine;
        final StringBuilder sBuffer = new StringBuilder();
        listOfQueries = new ArrayList<>();
        try (BufferedReader br = new BufferedReader(new InputStreamReader(ininSt))) {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "Error al tratar de abrir el archivo " + pathname + System.getProperty("line.separator") + inex);
            logger.error(inex);
            return;
        }
        ...
        try {
            for (String query : queries_01) {
                RSList_01.add(ConnCapip.getInstance().executeQuery(query));
            }
        } catch (final Exception inex) {
            JOptionPane.showMessageDialog(null, "Error al trar de ejecutar el comando sql" + System.getProperty("line.separator") + inex);
            logger.error(inex);
        }
        try {
