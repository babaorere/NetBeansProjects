 ...
 */
public final class BancosChequePagoCambiar extends javax.swing.JFrame {
    ...
    /**
     * Formulario para el cambio de cheque para una orden de pago
     *
     * @param inparent
     * @param inid_orden_pago
     */
    public BancosChequePagoCambiar(final java.awt.Window inparent, final long inid_orden_pago) {
        super();
        initComponents();
        parent = inparent;
        id_orden_pago = inid_orden_pago;
        setOwnBehavior();
        setCompBehavior();
        setStartConditions();
    }
    ...
    /**
     * Establece el comportamiento de la presente Ventana Rev 30/10/2016
     */
    ...
    private void setStartConditions() {
        ...
        try {
            final ResultSet rs = ConnCapip.getInstance().executeQuery("SELECT * FROM orden_pago WHERE id_orden_pago= " + id_orden_pago);
            if (rs.next()) {
                PagoModel reg = new PagoModel(rs);
                lblNumOrden.setText(String.valueOf(reg.getId()));
                txtNumAnt.setText(reg.getCheque());
                txtNumNuevo.requestFocusInWindow();
            } else {
                JOptionPane.showMessageDialog(null, "No existe ese número de orden: " + id_orden_pago);
                actSalir();
            }
        } catch (final Exception inex) {
            JOptionPane.showMessageDialog(null, "Error al tratar de recuperar la Orden de Pago: " + System.getProperty("line.separator") + inex);
            logger.error(inex);
        }
    }
    ...
    void initComponents() {
        ...
        // NOI18N
        txtNumAnt.setFont(new java.awt.Font("Tahoma", 3, 14));
        txtNumAnt.setHorizontalAlignment(javax.swing.JTextField.CENTER);
        txtNumAnt.setDisabledTextColor(new java.awt.Color(102, 0, 0));
        ...
        jLabel4.setLabelFor(txtNumNuevo);
        ...
        // NOI18N
        txtNumNuevo.setFont(new java.awt.Font("Arial", 3, 14));
        ...
        getContentPane().add(lblFondo, new org.netbeans.lib.awtextra.AbsoluteConstraints(-30, 0, 530, 390));
        pack();
...
//GEN-LAST:event_btnGuardarActionPerformed
    ...
    private void actGuardar() {
        ...
        try {
            ...
            final ResultSet rsPag = ConnCapip.getInstance().executeQuery("SELECT * FROM orden_pago WHERE id_orden_pago= " + id_orden_pago + " FOR UPDATE");
            if (rsPag.next()) {
                final PagoModel regPag = new PagoModel(rsPag);
                final PreparedStatement psOrdPag = ConnCapip.getInstance().getConnection().prepareStatement("UPDATE orden_pago set cheque= ? WHERE id_orden_pago= ?");
                psOrdPag.setString(1, sNumNuevo);
                psOrdPag.setLong(2, id_orden_pago);
                if (psOrdPag.executeUpdate() != 1) {
                    throw new Exception("Orden de pago, no actualizada");
                }
                final PreparedStatement psBanDet = ConnCapip.getInstance().getConnection().prepareStatement("UPDATE bancos_operaciones set soporte_o_cheque= ? WHERE (tipo_operacion= 'CH' OR tipo_operacion= 'SC') AND num_pag_ava= ? AND anulado_sn= 'N'");
                psBanDet.setString(1, setColStrLen("bancos_operaciones", "soporte_o_cheque", sNumNuevo));
                psBanDet.setLong(2, id_orden_pago);
                final int cantUp = psBanDet.executeUpdate();
                if (cantUp != 1) {
                ...
                pst.setString(1, regPag.getBanco());
                pst.setString(2, regPag.getCuenta());
                pst.setString(3, sNumAnt);
                pst.setString(4, sNumNuevo);
                pst.setString(5, com.principal.utils.Format.toStr(regPag.getA_pagar_bs()));
                // pag_ava
                pst.setString(6, "P");
                // num_pag_ava
                pst.setLong(7, regPag.getId());
                ...
                if (rsRegAnu.next()) {
                    regAnu = new BancosChAnuModel(rsRegAnu);
                } else {
                    regAnu = null;
                }
                // Generar los reportes
                rptPlanillaAnuPag(regPag, regAnu);
                PagoOrden.rptOrdPag(regPag.getId(), "ORDEN DE PAGO", true);
                PagoOrden.rptChPag(regPag.getId(), "ORDEN DE PAGO", true);
                ConnCapip.getInstance().EndTransaction();
                JOptionPane.showMessageDialog(null, "Cambio realizado");
                actSalir();
            } else {
                throw new Exception("Número de Pago inexistente: " + id_orden_pago);
            }
        } catch (final Exception inex) {
            ...
            JOptionPane.showMessageDialog(null, "Error al tratar de realizar los cambios" + System.getProperty("line.separator") + inex);
            logger.error(inex);
        }
    }

    @SuppressWarnings({"unchecked"})
    private void rptPlanillaAnuPag(final PagoModel inregPA, final BancosChAnuModel inregAnu) throws Exception {
     ...
     * @param evt
     */
    private void txtNumNuevoKeyTyped(java.awt.event.KeyEvent evt) {
//GEN-FIRST:event_txtNumNuevoKeyTyped
...
//GEN-LAST:event_txtMotivoKeyTyped
    ...
    public static void main(String[] args) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            ...
            @Override
            public void run() {
                new BancosChequePagoCambiar(null, 13).setVisible(true);
            }
        });
    }
    ...
    private static final Logger logger = LogManager.getLogger(BancosChequePagoCambiar.class);
}
