...
//GEN-FIRST:event_btnCambiarActionPerformed
        ...
        // We check if the stored password is null
        byte[] dbPassDigest = null;
        byte[] txtPassDigest = null;
        try {
            final ResultSet rs = ConnCapip.getInstance().executeQuery("SELECT pass FROM usuario where user='" + user + "'");
            if (rs.next()) {
                dbPassDigest = rs.getBytes(1);
                if (dbPassDigest != null) {
                    int len = txtPass.getPassword().length;
                    if ((len < 4) || (len > 10)) {
                        JOptionPane.showMessageDialog(null, "Longitud de contraseña de 4 hasta 10 caracteres");
                        txtPass.requestFocusInWindow();
                        return;
                    }
                    txtPassDigest = MessageDigest.getInstance("MD5").digest(charToByte(txtPass.getPassword()));
                }
            } else {
        ...
        try {
            if ((dbPassDigest != null) && (txtPassDigest != null)) {
                if (txtPassDigest.length != dbPassDigest.length) {
                    JOptionPane.showMessageDialog(null, "Usuario/Contraseña invalido(a)");
                    return;
                }
                for (int i = 0; i < txtPassDigest.length; i++) {
                    if (txtPassDigest[i] != dbPassDigest[i]) {
                        JOptionPane.showMessageDialog(null, "Usuario/Contraseña invalido(a)");
                        return;
                    }
                }
            }
            int lenNew = txtNewPass.getPassword().length;
            if ((lenNew < 4) || (lenNew > 10)) {
                JOptionPane.showMessageDialog(null, "Longitud de contraseña de 4 hasta 10 caracteres");
                txtNewPass.requestFocusInWindow();
                return;
            }
            int lenConf = txtConfPass.getPassword().length;
            if ((lenConf < 4) || (lenConf > 10)) {
                JOptionPane.showMessageDialog(null, "Longitud de contraseña de 4 hasta 10 caracteres");
...
//GEN-LAST:event_btnCambiarActionPerformed
    ...
    private static final Logger logger = LogManager.getLogger(UserPassChange.class);
}
