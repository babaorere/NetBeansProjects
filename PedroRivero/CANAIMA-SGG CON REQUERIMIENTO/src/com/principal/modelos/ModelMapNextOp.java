...
public final class ModelMapNextOp {
    ...
    public static long getNext(final long inejeFis, final String inclaveParcial) throws Exception {
        ...
        for (int i = 0; i < 3; i++) {
            try {
                rsSec = ConnCapip.getInstance().getConnection().prepareStatement("SELECT * FROM map_next_id WHERE clave= '" + strClave + "' FOR UPDATE", ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_UPDATABLE).executeQuery();
                break;
            } catch (final Exception inex) {
                rsSec = null;
                Random rnd = new Random();
                rnd.setSeed(Globales.getServerTimeStamp().getTime());
                // Espera hasta 100msg, hasta volverlo a intentar
                TimeUnit.MILLISECONDS.sleep((int) (rnd.nextDouble() * 100));
                logger.error(inex);
            }
        }
        if (rsSec == null) {
        ...
        if (rsSec.next()) {
            ...
            secuencial = 1;
            final PreparedStatement pstSecuencial = ConnCapip.getInstance().getConnection().prepareStatement("INSERT INTO map_next_id" + "(clave, valor, id_user) " + "VALUES (?, ?, ?)");
    ...
    private static final Logger logger = LogManager.getLogger(ModelMapNextOp.class);
}
