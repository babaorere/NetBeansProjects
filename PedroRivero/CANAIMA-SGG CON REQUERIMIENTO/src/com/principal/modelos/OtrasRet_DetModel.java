 ...
 */
public final class OtrasRet_DetModel {

    private final long id_otras_ret_det;
     ...
     *
     */
    public OtrasRet_DetModel(final ResultSet inrs) throws Exception {
        id_otras_ret_det = inrs.getLong("id_otras_ret_det");
    ...
    public String toString() {
        return String.valueOf(id_otras_ret_det);
    }
     ...
     * @throws Exception
     */
    public static ArrayList<OtrasRet_DetModel> getxId_causado(final long id_causado) throws Exception {
        ArrayList<OtrasRet_DetModel> list = new ArrayList<>();
        final ResultSet rs = ConnCapip.getInstance().executeQuery("SELECT * FROM otras_ret_det WHERE id_causado= " + id_causado);
        while (rs.next()) {
            list.add(new OtrasRet_DetModel(rs));
        }
        return list;
    }
     ...
     * @throws Exception
     */
    public static ArrayList<OtrasRet_DetModel> getxId_retencion(final long inid) throws Exception {
        ArrayList<OtrasRet_DetModel> list = new ArrayList<>();
        final ResultSet rs = ConnCapip.getInstance().executeQuery("SELECT * FROM otras_ret_det WHERE id_otras_ret= " + inid);
        while (rs.next()) {
            list.add(new OtrasRet_DetModel(rs));
        }
        return list;
     ...
     * @throws Exception
     */
    public static OtrasRet_DetModel getxId(final long inid) throws Exception {
        final ResultSet rs = ConnCapip.getInstance().executeQuery("SELECT * FROM otras_ret_det WHERE id_otras_ret_det= " + inid);
        if (rs.next()) {
            return new OtrasRet_DetModel(rs);
        }
        return null;
    }
     ...
     */
    public long getId_otras_ret_det() {
        return id_otras_ret_det;
    }
    ...
    private static final Logger logger = LogManager.getLogger(OtrasRet_DetModel.class);
}
