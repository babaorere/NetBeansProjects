...
public final class Compromiso extends javax.swing.JFrame {
     ...
     * @param inparent
     */
    public Compromiso(final java.awt.Window inparent) {
        super();
        initComponents();
     ...
     * @param intxtSubTotal
     */
    public static void setTxtCantBehavior(final JFormattedTextField intxtCant, final JFormattedTextField intxtPUnitario, final JFormattedTextField intxtSubTotal) {
     ...
     * @param intxtSubTotal
     */
    public static void setTxtPUnitarioBehavior(final JFormattedTextField intxtCant, final JFormattedTextField intxtPUnitario, final JFormattedTextField intxtSubTotal) {
        // Para que cada vez que se modifique el campo se actualice el subtotal
     ...
     * @param intxtSubTotal
     */
    public static void newSubTotal(final JFormattedTextField intxtCant, final JFormattedTextField intxtPUnitario, final JFormattedTextField intxtSubTotal) {

        SwingUtilities.invokeLater(() -> {

            BigDecimal bdCant;
            String x1 = intxtPUnitario.getText();
            try {
                intxtCant.commitEdit();
                bdCant = (BigDecimal) intxtCant.getValue();
            } catch (final Exception inex) {
                intxtSubTotal.setText("0,00");
                intxtCant.setValue(BigDecimal.ZERO);
                logger.error(inex);
                return;
            }

            if (bdCant == null) {
                bdCant = BigDecimal.ZERO;
            }
            ...
            try {
                ...
                intxtSubTotal.setText("0,00");
                intxtPUnitario.setValue(BigDecimal.ZERO);
                logger.error(inex);
                return;
            }

            if (bdPU == null) {
                bdPU = BigDecimal.ZERO;
            }
            BigDecimal rest = bdCant.multiply(bdPU);
            intxtSubTotal.setText(Format.toStr(rest));
            intxtSubTotal.setValue(rest);
        });
    }
     ...
     * @param evt
     */
    private void btnOtrosComprActionPerformed(java.awt.event.ActionEvent evt) {
//GEN-FIRST:event_btnOtrosComprActionPerformed
...
//GEN-FIRST:event_btnIVAActionPerformed
        ...
        try {
            ...
            JOptionPane.showMessageDialog(this, "Error al recuperar el IVA" + System.getProperty("line.separator") + inex);
            logger.error(inex);
            return;
        }
        //
...
//GEN-LAST:event_btnGuardarActionPerformed
     ...
     * @param inparam
     * @param intblItem
     * @param inIVA_DEC_VALUE
     * @throws Exception
     */
    public static void genMaster(final Map<String, Object> inparam, final JTable intblItem, BigDecimal inIVA_DEC_VALUE) throws Exception {
        try (final PreparedStatement pst = ConnCapip.getInstance().getConnection().prepareStatement(// 1, 2, 3, 4, 5
                "INSERT INTO " + ((TipoCompr) inparam.get("tipo_compr")).getTbl()
                + // 6, 7, 8, 9
                "(id_compr, id_user, id_session, id_causado, id_orden_pago, "
                + // 10, 11, 12, 13, 14
                "fecha_compr, date_session, benef_razonsocial, benef_rif_ci, "
                + // 15, 16, 17, 18
                "observacion, num_fact, num_control, fecha_fact, total_bs, "
                + // 19, 20, 21
                "base_imponible_bs, iva_grav_bs, iva_porc_aplic, iva_porc_ret, "
                + // 22, 23
                "islr_grav_bs, islr_porc_ret, ejefis, " + "oret_grav_bs, oret_bs) " + "VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
            pst.setLong(1, (long) inparam.get("id_compr"));
            ...
            // id_pago
            pst.setLong(5, 0);
            pst.setDate(6, (java.sql.Date) inparam.get("fecha_compr"));
            ...
            pst.setString(11, (String) inparam.get("num_fact"));
            pst.setString(12, (String) inparam.get("num_control"));
            pst.setDate(13, (java.sql.Date) inparam.get("fecha_fact"));
            pst.setBigDecimal(14, (BigDecimal) inparam.get("total_bs"));
            BigDecimal base_imponible = (BigDecimal) inparam.get("base_imponible_bs");
            pst.setBigDecimal(15, base_imponible);
            pst.setBigDecimal(16, (BigDecimal) inparam.get("iva_grav_bs"));
            // iva_porc_aplic
            pst.setBigDecimal(17, inIVA_DEC_VALUE.movePointRight(2));
            // iva_porc_ret
            pst.setBigDecimal(18, BigDecimal.ZERO);
            // islr_grav_bs
            pst.setBigDecimal(19, (BigDecimal) inparam.get("islr_grav_bs"));
            // islr_porc_ret
            pst.setBigDecimal(20, BigDecimal.ZERO);
     ...
     * @throws Exception
     */
    public static void anuMaster(final long inid_compr, final TipoCompr intipo_compr) throws Exception {
        try (final PreparedStatement pst = ConnCapip.getInstance().getConnection().prepareStatement("UPDATE " + intipo_compr.getTbl() + " SET anulado_sn= 'S' WHERE id_compr = " + inid_compr)) {
            if (pst.executeUpdate() != 1) {
     ...
     * @throws Exception
     */
    public static void genDetail(final Map<String, Object> inparam, JTable intblItem) throws Exception {
        final long id_compr = (long) inparam.get("id_compr");
        final TipoCompr tipo_compr = (TipoCompr) inparam.get("tipo_compr");
        for (int i = 0; i < intblItem.getRowCount(); i++) {
            final BigDecimal cantpro = BigDecimal.valueOf((double) intblItem.getValueAt(i, COL_ITEM_CANT));
            final String descPro = setColStrLen("compr_det", "descpro", (String) intblItem.getValueAt(i, COL_ITEM_DESC));
            final BigDecimal punitario = BigDecimal.valueOf((double) intblItem.getValueAt(i, COL_ITEM_PUNITARIO));
            final String codpresupe = (String) intblItem.getValueAt(i, COL_ITEM_CODIGO);
            final String partida = (String) intblItem.getValueAt(i, COL_ITEM_PARTIDA);
            genComprPttoE(codpresupe, Format.toBigDec((double) intblItem.getValueAt(i, 3)));
            try (final PreparedStatement pst = ConnCapip.getInstance().getConnection().prepareStatement(// 1, 2, 3, 4
     ...
     * @throws Exception
     */
    public static void anuDetail(final long inid_compr, final TipoCompr intipo_compr) throws Exception {
        final ResultSet rsComprDet = ConnCapip.getInstance().executeQuery("SELECT * FROM compr_det " + "WHERE id_compr= " + inid_compr + " AND tipo_compr= '" + intipo_compr.name() + "'");
    ...
    public void genReport(final long inid, final Map<String, Object> inparam, final boolean inexport) throws Exception {
        ...
        final InputStream pathRpt = getClass().getResourceAsStream("/reportes/Compromiso.jasper");
            ...
            new Thread() {
                ...
                public void run() {
                    try {
                        ...
                        final JasperPrint jasperPrint = JasperFillManager.fillReport((JasperReport) JRLoader.loadObject(pathRpt), new HashMap<>(param), ConnCapip.getInstance().getConnection());
    ...
    public static String getNombreSinExtension(String nombreArchivo) {
        int lastDotIndex = nombreArchivo.lastIndexOf(".");
        if (lastDotIndex != -1) {
            return nombreArchivo.substring(0, lastDotIndex);
        } else {
...
//GEN-FIRST:event_btnAnularActionPerformed
...
//GEN-LAST:event_txtNumControlActionPerformed
    private void chkISLRActionPerformed(java.awt.event.ActionEvent evt) {
//GEN-FIRST:event_chkISLRActionPerformed
...
//GEN-LAST:event_chkISLRActionPerformed
     ...
     * @param incmb
     */
    public static void cargarComboIva(final JComboBox<String> incmb) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (regPpto != null) {
                    ...
                    JOptionPane.showMessageDialog(null, "Error en Partida asociada al IVA " + Format.toStr(regIva.getValor_porc()) + "%" + System.getProperty("line.separator") + "Aún no se encuentra cargada en el Presupuesto" + System.getProperty("line.separator") + "Trate cargando dicha partida, o reestableciendo el IVA Aplicado");
                    return;
                }
            }
        } catch (final Exception inex) {
        ...
        incmb.setModel(modeloCombo);
        if (incmb.getItemCount() >= 2) {
            incmb.setSelectedIndex(1);
        } else {
            incmb.setSelectedIndex(0);
        }
    }
