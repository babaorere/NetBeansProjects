...
import com.principal.configuracion.RegistroBenef;
...
import static com.principal.modelos.PresupeModel.genComprPttoE;
...
public final class CompromisoEditar extends javax.swing.JFrame {
     ...
     *
     * @param inparent
     * @param inid_orden_compr
     * @param intipo_compr
     */
    public CompromisoEditar(final java.awt.Window inparent, final long inid_orden_compr, final TipoCompr intipo_compr) {
        super();
        initComponents();
        parent = inparent;
        regCompr = LoadCompr(inid_orden_compr, intipo_compr);
        listComprDet = LoadComprDet(regCompr, intipo_compr);
        tipo_compr = intipo_compr;
        setOwnBehavior();
        setCompBehavior();
        setStartConditions();
    }
     ...
     * @param inaction
     */
    public static void setTblBenefBehavior(final JTable intblBenef, AbstractAction inaction) {
        // Configurar la tabla de los Beneficiarios, para manejar el ENTER
        intblBenef.getInputMap(JTable.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0), "action.enter");
        intblBenef.getActionMap().put("action.enter", inaction);
        // Configurar la tabla de los Beneficiarios, para manejar el SPACE
        intblBenef.getInputMap(JTable.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(KeyStroke.getKeyStroke(KeyEvent.VK_SPACE, 0), "action.space");
        intblBenef.getActionMap().put("action.space", inaction);
        DefaultTableCellRenderer tcr;
        tcr = new DefaultTableCellRenderer();
        tcr.setHorizontalAlignment(SwingConstants.CENTER);
        intblBenef.getColumnModel().getColumn(COL_BENEF_RIF_CI).setCellRenderer(tcr);
        intblBenef.setDefaultRenderer(Integer.class, new IntegerCellRenderer());
        intblBenef.setDefaultRenderer(Long.class, new IntegerCellRenderer());
        intblBenef.setDefaultRenderer(Double.class, new DecimalCellRenderer());
    }
     ...
     * @param intblItem
     */
    public static void setTblItemBehavior(JTable intblItem) {
        DefaultTableCellRenderer tcr;
        tcr = new DefaultTableCellRenderer();
        tcr.setHorizontalAlignment(SwingConstants.CENTER);
        intblItem.getColumnModel().getColumn(COL_ITEM_CODIGO).setCellRenderer(tcr);
        intblItem.setDefaultRenderer(Integer.class, new IntegerCellRenderer());
        intblItem.setDefaultRenderer(Long.class, new IntegerCellRenderer());
        intblItem.setDefaultRenderer(Double.class, new DecimalCellRenderer());
    }
     ...
     * @param inaction
     */
    public static void setTblPartidaBehavior(JTable intblPartida, final AbstractAction inaction) {
        // Configurar la tabla de las Partidas, para manejar el ENTER
        intblPartida.getInputMap(JTable.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0), "action.enter");
        intblPartida.getActionMap().put("action.enter", inaction);
        // Configurar la tabla de las Partidas, para manejar el SPACE
        intblPartida.getInputMap(JTable.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(KeyStroke.getKeyStroke(KeyEvent.VK_SPACE, 0), "action.space");
        intblPartida.getActionMap().put("action.space", inaction);
        // Configurar la tabla de partidas
        intblPartida.setDefaultRenderer(Integer.class, new IntegerCellRenderer());
        intblPartida.setDefaultRenderer(Long.class, new IntegerCellRenderer());
        intblPartida.setDefaultRenderer(Double.class, new DecimalCellRenderer());
    }

    /**
     * Rev. 13/09/2016
     *
     * @param intxtFechaFact
     */
    public static void setTxtFechaFactBehavior(final JFormattedTextField intxtFechaFact) {
        // Establecer el formateador de fecha
        intxtFechaFact.setFormatterFactory(Globales.getDateFormatterFactory());
    }
     ...
     *
     * @param intxtCant
     * @param intxtPUnitario
     * @param intxtSubTotal
     */
    public static void setTxtCantBehavior(final JFormattedTextField intxtCant, final JFormattedTextField intxtPUnitario, final JTextField intxtSubTotal) {
        // Para que cada vez que se modifique el campo se actualice el subtotal
        intxtCant.getDocument().addDocumentListener(new DocumentListener() {

            @Override
            public void changedUpdate(DocumentEvent e) {
                newSubTotal(intxtCant, intxtPUnitario, intxtSubTotal);
            }
     ...
     * @param intxtSubTotal
     */
    public static void setTxtPUnitarioBehavior(final JFormattedTextField intxtCant, final JFormattedTextField intxtPUnitario, final JTextField intxtSubTotal) {
        // Para que cada vez que se modifique el campo se actualice el subtotal
        intxtPUnitario.getDocument().addDocumentListener(new DocumentListener() {
     ...
     * @param intxtSubTotal
     */
    public static void newSubTotal(final JFormattedTextField intxtCant, final JFormattedTextField intxtPUnitario, final JTextField intxtSubTotal) {
        final BigDecimal bdCant;
        try {
            intxtCant.commitEdit();
            bdCant = (BigDecimal) intxtCant.getValue();
        } catch (final Exception inex) {
            intxtSubTotal.setText("0,00");
            logger.error(inex);
            return;
        }
        final BigDecimal bdPU;
        try {
            intxtPUnitario.commitEdit();
            bdPU = (BigDecimal) intxtPUnitario.getValue();
        } catch (final Exception inex) {
            intxtSubTotal.setText("0,00");
            logger.error(inex);
            return;
        }
        intxtSubTotal.setText(Format.toStr(bdCant.multiply(bdPU)));
    }
    ...
    public void setStartConditions() {
        if (regCompr != null) {
            ...
            try {
                ...
                JOptionPane.showMessageDialog(this, "Error al tratar de recuperar el Beneficiario" + System.getProperty("line.separator") + inex);
                actSalir();
                logger.error(inex);
                return;
            }
        ...
        //iva_grav_bs.compareTo(BigDecimal.ZERO) == 0);
     ...
     * @return
     */
    private ComprModel LoadCompr(final long inid_orden_compr, final TipoCompr intipo_compr) {
        final ComprModel auxRegCompr;
        // verificar que el compromiso exista y no este anulado
        // Verificar la existencia de la orden
        try {
            final ResultSet rsCompr = ConnCapip.getInstance().executeQuery("SELECT * FROM " + intipo_compr.getTbl() + " WHERE id_compr= " + inid_orden_compr);
            if (rsCompr.next()) {
                auxRegCompr = new ComprModel(rsCompr);
                final Map<String, Object> param = new HashMap<>(101);
                param.put("tipo_compr", intipo_compr);
     ...
     * @return
     */
    private ArrayList<ComprDetModel> LoadComprDet(final ComprModel inregCompr, final TipoCompr intipo_compr) {
        final ArrayList<ComprDetModel> list = new ArrayList<>();
        try {
            final ResultSet rsComprDet = ConnCapip.getInstance().executeQuery("SELECT * FROM compr_det WHERE id_compr= " + inregCompr.getId_compromiso() + " AND tipo_compr= '" + intipo_compr.name() + "'");
     ...
     * @param intblBenef
     */
    public static void UpdateTblBenef(JTable intblBenef) {
        intblBenef.clearSelection();
        final DefaultTableModel model = (DefaultTableModel) intblBenef.getModel();
        ...
        if (intblBenef.getRowCount() > 0) {
            intblBenef.setRowSelectionInterval(0, 0);
            intblBenef.scrollRectToVisible(intblBenef.getCellRect(0, 0, true));
        }
    }
     ...
     * @param intblPartida
     */
    public static void UpdateTblPartida(final JTable intblPartida) {
        intblPartida.clearSelection();
        final DefaultTableModel model = (DefaultTableModel) intblPartida.getModel();
        model.getDataVector().removeAllElements();
        model.fireTableDataChanged();
        try {
            final Object[] datos = new Object[3];
            final ResultSet rs = ConnCapip.getInstance().executeQuery("SELECT * FROM presupe WHERE ejefis=" + Globales.getEjeFisYear() + " AND anulado_sn= 'N' ORDER BY codigo ASC");
            while (rs.next()) {
        ...
        if (intblPartida.getRowCount() > 0) {
            intblPartida.setRowSelectionInterval(0, 0);
            intblPartida.scrollRectToVisible(intblPartida.getCellRect(0, 0, true));
        }
    }
    ...
    void initComponents() {
        ...
        // NOI18N
        btnOtrosCompr.setFont(new java.awt.Font("Arial", 2, 18));
        ...
        // NOI18N
        txtRazonSocial.setFont(new java.awt.Font("Arial", 3, 14));
        ...
        // NOI18N
        txtDomicilio.setFont(new java.awt.Font("Arial", 3, 14));
        ...
        // NOI18N
        txtTelefonos.setFont(new java.awt.Font("Arial", 3, 14));
        ...
        // NOI18N
        txtNroFact.setFont(new java.awt.Font("Arial", 3, 14));
        ...
        // NOI18N
        txtFechaFact.setFont(new java.awt.Font("Arial", 3, 14));
        ...
        // NOI18N
        txtCant.setFont(new java.awt.Font("Arial", 3, 14));
        ...
        // NOI18N
        txtDesc.setFont(new java.awt.Font("Arial", 3, 14));
        ...
        // NOI18N
        txtCodPartida.setFont(new java.awt.Font("Arial", 3, 18));
        ...
        // NOI18N
        tblPartida.setFont(new java.awt.Font("Arial", 3, 12));
        tblPartida.setModel(new javax.swing.table.DefaultTableModel(new Object[][] {}, new String[] { "Código", "Partida", "Monto Bs." }) {
        ...
        // NOI18N
        txaConcepto.setFont(new java.awt.Font("Arial", 3, 12));
        ...
        btnEliminarUlt.setBackground(java.awt.SystemColor.inactiveCaption);
        // NOI18N
        btnEliminarUlt.setFont(new java.awt.Font("Arial", 2, 18));
        btnEliminarUlt.setText("ELIM. ULT.");
        btnEliminarUlt.setBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(0, 0, 0)));
        ...
        // NOI18N
        txtSubTotal.setFont(new java.awt.Font("Arial", 3, 14));
        jPanel1.add(txtSubTotal, new org.netbeans.lib.awtextra.AbsoluteConstraints(670, 50, 198, 31));
        // NOI18N
        txtPUnitario.setFont(new java.awt.Font("Arial", 3, 14));
        ...
        chkISLR.setBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(0, 0, 0)));
        ...
        // NOI18N
        cmbIva.setFont(new java.awt.Font("Arial", 2, 18));
        ...
        // NOI18N
        tblBenef.setFont(new java.awt.Font("Arial", 3, 12));
        tblBenef.setModel(new javax.swing.table.DefaultTableModel(new Object[][] {}, new String[] { "BENEFICIARIO", "RIF / CI", "DOMICILIO", "TELÉFONOS" }) {
        ...
        // NOI18N
        jLabel15.setFont(new java.awt.Font("Arial", 3, 24));
        ...
        // NOI18N
        lblTipoCompr.setFont(new java.awt.Font("Arial", 3, 36));
        lblTipoCompr.setForeground(new java.awt.Color(255, 255, 255));
        // NOI18N
        lblTipoCompr.setDisabledIcon(new javax.swing.ImageIcon(getClass().getResource("/imagenes/anular_32x32.png")));
        getContentPane().add(lblTipoCompr, new org.netbeans.lib.awtextra.AbsoluteConstraints(820, 80, 130, 60));
        // NOI18N
        txtRIF_CI.setFont(new java.awt.Font("Arial", 3, 14));
        ...
        // NOI18N
        txtTotalBs.setFont(new java.awt.Font("Arial", 3, 32));
        ...
        // NOI18N
        txtID_Compr.setFont(new java.awt.Font("Arial", 3, 36));
        ...
        // NOI18N
        txtFechaCompr.setFont(new java.awt.Font("Arial", 3, 24));
        txtFechaCompr.addActionListener(new java.awt.event.ActionListener() {

            public void actionPerformed(java.awt.event.ActionEvent evt) {
                txtFechaComprActionPerformed(evt);
            }
        });
        ...
        // NOI18N
        btnBenefNuevo.setFont(new java.awt.Font("Arial", 3, 16));
        ...
        getContentPane().add(btnBenefNuevo, new org.netbeans.lib.awtextra.AbsoluteConstraints(1050, 175, 90, 20));
        btnRequisicion.setBackground(java.awt.SystemColor.inactiveCaption);
        // NOI18N
        btnRequisicion.setIcon(new javax.swing.ImageIcon(getClass().getResource("/imagenes/requisicion_32x32.png")));
        ...
        // NOI18N
        btnEditar.setIcon(new javax.swing.ImageIcon(getClass().getResource("/imagenes/editar_32x32.png")));
        ...
        btnEliminarCompr.setBackground(java.awt.SystemColor.inactiveCaption);
        // NOI18N
        btnEliminarCompr.setIcon(new javax.swing.ImageIcon(getClass().getResource("/imagenes/anular_32x32.png")));
...
//GEN-LAST:event_tblBenefMouseClicked
    ...
    boolean actInsertIVA() {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(this, "Error al verificar la disponibilidad presupuestaria" + System.getProperty("line.separator") + inex);
            logger.error(inex);
            return false;
        }
...
//GEN-FIRST:event_btnConsultarActionPerformed
        if (tblItem.getRowCount() > 0) {
            if (JOptionPane.showConfirmDialog(this, "¿Los datos cargados se perderan. Desea continuar con la Consulta ?", "Confirmar acción", JOptionPane.YES_NO_OPTION) != JOptionPane.OK_OPTION) {
                return;
            }
        }
...
//GEN-LAST:event_btnConsultarActionPerformed
     ...
     * @param evt
     */
    private void btnEliminarUltActionPerformed(java.awt.event.ActionEvent evt) {
//GEN-FIRST:event_btnEliminarUltActionPerformed
...
//GEN-LAST:event_btnGuardarActionPerformed
     ...
     * @return
     */
    private boolean valComp(final Map<String, Object> inparam) {
        ...
        inparam.put("iva_porc", iva_aplic.getValor_porc());
        ...
        if (chkISLR.isSelected()) {
            ...
            inparam.put("islr_grav_bs", BigDecimal.ZERO);
        }
        inparam.put("islr_porc_ret", BigDecimal.ZERO);
        ...
        for (String tbl : arrTbl) {
            try {
                final ResultSet rs = ConnCapip.getInstance().executeQuery("SELECT * FROM " + tbl + " WHERE benef_rif_ci = '" + benef_rif_ci + "' AND num_fact = '" + num_fact + "' AND id_compr != " + regCompr.getId_compromiso());
                if (rs.next()) {
     ...
     *
     * @param inparam
     * @param intblItemtblItem@throws Exception
     */
    private void genMaster(final Map<String, Object> inparam) throws Exception {
        final PreparedStatement pst = ConnCapip.getInstance().getConnection().prepareStatement(// 1, 2, 3, 4
                "UPDATE " + tipo_compr.getTbl() + " "
                + // 5, 6, 7, 8
                "SET id_user= ?, id_session= ?, id_causado= ?, id_orden_pago= ?, "
                + // 9, 10, 11, 12, 13
                "fecha_compr= ?, date_session= ?, benef_razonsocial= ?, benef_rif_ci= ?, "
                + // 14, 15, 16, 17
                "observacion= ?, num_fact= ?, , num_control= ?, fecha_fact= ?, total_bs= ?, "
                + // 19, 19, 20
                "base_imponible_bs= ?, iva_grav_bs= ?, iva_porc_aplic= ?, iva_porc_ret= ?, " + "islr_grav_bs= ?, islr_porc_ret= ?, ejefis= ? "
                + // 21
                "WHERE id_compr= ?", Statement.RETURN_GENERATED_KEYS);
        ...
        // id_pago
        pst.setLong(4, 0);
        pst.setDate(5, (java.sql.Date) inparam.get("fecha_compr"));
        pst.setTimestamp(6, UserPassIn.getDateSession());
        pst.setString(7, (String) inparam.get("benef_razonsocial"));
        pst.setString(8, (String) inparam.get("benef_rif_ci"));
        pst.setString(9, (String) inparam.get("observacion"));
        pst.setString(10, (String) inparam.get("num_fact"));
        pst.setString(11, (String) inparam.get("num_control"));
        pst.setDate(12, (java.sql.Date) inparam.get("fecha_fact"));
        pst.setBigDecimal(13, (BigDecimal) inparam.get("total_bs"));
        BigDecimal base_imponible = (BigDecimal) inparam.get("base_imponible_bs");
        pst.setBigDecimal(14, (BigDecimal) inparam.get("base_imponible_bs"));
        pst.setBigDecimal(15, (BigDecimal) inparam.get("iva_grav_bs"));
        // iva_porc_aplic
        pst.setBigDecimal(16, (BigDecimal) inparam.get("iva_porc"));
        // iva_porc_ret
        pst.setBigDecimal(17, BigDecimal.ZERO);
        // islr_grav_bs
        pst.setBigDecimal(18, (BigDecimal) inparam.get("islr_grav_bs"));
        // islr_porc_ret
        pst.setBigDecimal(19, BigDecimal.ZERO);
        pst.setDate(20, Globales.getEjefis());
        pst.setLong(21, regCompr.getId_compromiso());
        if (pst.executeUpdate() != 1) {
            throw new Exception("Error al insertar el registro");
        }
        inparam.put("id_compr", regCompr.getId_compromiso());
    }
    ...
    private void genDetail() throws Exception {
        ...
        for (ComprDetModel regDet : listComprDet) {
            final PreparedStatement pst = ConnCapip.getInstance().getConnection().prepareStatement("DELETE FROM compr_det " + "WHERE id_compr_det = ?");
        ...
        for (int i = 0; i < tblItem.getRowCount(); i++) {
            ...
            final PreparedStatement pst = ConnCapip.getInstance().getConnection().prepareStatement(// 1, 2, 3, 4
                    "INSERT INTO compr_det"
                    + // 5, 6, 7
                    "(id_compr, tipo_compr, cantpro, descpro, " + "punitario, codpresu, partida) " + "VALUES ( ?, ?, ?, ?, ?, ?, ?)");
     ...
     * @throws Exception
     */
    public static void genReport(final long inid, final Map<String, Object> inparam, final boolean inexport) throws Exception {
        final TipoCompr tipo_compr = (TipoCompr) inparam.get("tipo_compr");
        final ComprModel compr = ComprModel.getxID(inid, tipo_compr);
        if (compr == null) {
            JOptionPane.showMessageDialog(null, "Registro no encontrado");
            return;
        }
        ...
        if (pathRpt == null) {
            ...
            new Thread() {
                ...
                public void run() {
                    try {
                        final JasperPrint jasperPrint = JasperFillManager.fillReport(pathRpt, new HashMap<>(param), ConnCapip.getInstance().getConnection());
    ...
    private void tblPartidaMouseClicked(java.awt.event.MouseEvent evt) {
//GEN-FIRST:event_tblPartidaMouseClicked
...
//GEN-LAST:event_txtDescKeyTyped
    private void btnBenefNuevoActionPerformed(java.awt.event.ActionEvent evt) {
//GEN-FIRST:event_btnBenefNuevoActionPerformed
        java.awt.Window me = this;
        java.awt.EventQueue.invokeLater(new Runnable() {
...
//GEN-LAST:event_btnBenefNuevoActionPerformed
    private void btnEliminarComprActionPerformed(java.awt.event.ActionEvent evt) {
//GEN-FIRST:event_btnEliminarComprActionPerformed
        if (JOptionPane.showConfirmDialog(this, "La operación de anulación de Compromiso es irreversible. ¿ Desea continuar ?", "Confirmar acción", JOptionPane.YES_NO_OPTION) != JOptionPane.OK_OPTION) {
            return;
        }
        actEliminar();
    }
...
//GEN-LAST:event_txtNumControlActionPerformed
    private void txtFechaComprActionPerformed(java.awt.event.ActionEvent evt) {
//GEN-FIRST:event_txtFechaComprActionPerformed
        // TODO add your handling code here:
    }

//GEN-LAST:event_txtFechaComprActionPerformed
    private void actEliminar() {
        try {
            // Verificar la existencia de la orden
            final ResultSet rsCompr = ConnCapip.getInstance().executeQuery("SELECT * FROM " + tipo_compr.getTbl() + " WHERE id_compr= " + regCompr.getId_compromiso());
            if (rsCompr.next()) {
                ...
                if (JOptionPane.showConfirmDialog(this, "Esta operación es irreversible. Desea eliminar el registro ?", "Confirmar acción", JOptionPane.YES_NO_OPTION) == JOptionPane.OK_OPTION) {
                    try {
                        ConnCapip.getInstance().BeginTransaction();
                        final ResultSet rsComprDet = ConnCapip.getInstance().executeQuery("SELECT * FROM compr_det WHERE id_compr= " + regCompr.getId_compromiso() + " AND tipo_compr= '" + tipo_compr.name() + "'");
                        while (rsComprDet.next()) {
                        ...
                        final PreparedStatement pst = ConnCapip.getInstance().getConnection().prepareStatement("UPDATE " + tipo_compr.getTbl() + " SET anulado_sn= 'S' WHERE id_compr= " + regCompr.getId_compromiso());
                        if (pst.executeUpdate() != 1) {
    ...
    private static final Logger logger = LogManager.getLogger(CompromisoEditar.class);
}
