...
public final class PagoReport extends XJFrameJTable {
     ...
     * @param _parent
     */
    public PagoReport(final java.awt.Window _parent) {
        super(_parent);
        initComponents();
    ...
    private void btnReportSummaryActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnReportSummaryActionPerformed
        ...
        try {
            Conn.getConnection().prepareStatement("DELETE FROM ordenpago_rpt_summary WHERE fchsession <= '" + fchDelete + "' OR iduser=" + UserPassIn.getIdUser() + " AND idsession=" + UserPassIn.getIdSession()).executeUpdate();
        } catch (final Exception _ex) {
            JOptionPane.showMessageDialog(null, _ex);
            return;
        }
        ...
        for (int i = 0; i < tblMaster.getRowCount(); i++) {
            ...
            try {
                final PreparedStatement pst = Conn.getConnection().prepareStatement("INSERT INTO ordenpago_rpt_summary"
    ...
    @SuppressWarnings("unchecked")
    public void reportSummary(BigDecimal _tot) {
        if (tblMaster.getRowCount() <= 0) {
