...
public class Otras_Anular extends javax.swing.JFrame {
     ...
     * @param _parent
     */
    public Otras_Anular(final java.awt.Window _parent) {
        super();
        initComponents();
    ...
    private void actAnular() {
        ...
        try {
            ...
            final ResultSet rsRet = Conn.executeQuery("SELECT * FROM otras_ret WHERE id_otras_ret= " + id);
            if (rsRet.next()) {
                ...
                final ResultSet rsRetDet = Conn.executeQuery("SELECT * FROM otras_ret_det WHERE id_otras_ret= " + regRet.getId_otras_ret());
                while (rsRetDet.next()) {
                ...
                if (JOptionPane.showConfirmDialog(this, "Esta operación es irreversible. Desea anular la Retención ?",
                    ...
                    try {
                        ...
                        final PreparedStatement pstDet = Conn.getConnection().prepareStatement("UPDATE otras_ret_det SET anulado_sn= 'S' WHERE id_otras_ret= " + id);
                        if (pstDet.executeUpdate() <= 0) {
                            throw new Exception("Error al tratar de actualizar el Registro");
                        }

                        // Actualizar la Retencion
                        final PreparedStatement pst = Conn.getConnection().prepareStatement("UPDATE otras_ret SET anulado_sn= 'S' WHERE id_otras_ret= " + id);
                        if (pst.executeUpdate() != 1) {
                            throw new Exception("Error al tratar de actualizar el Registro");
                        }

                        // Actualizar todos los Causado asociados
                        final ResultSet rsRetDet2 = Conn.executeQuery("SELECT * FROM otras_ret_det WHERE id_otras_ret= " + id);
                        while (rsRetDet2.next()) {
                            final IslrRetencionDetModel regRetDet = new IslrRetencionDetModel(rsRetDet2);

                            final PreparedStatement pstCau = Conn.getConnection().prepareStatement("UPDATE causado SET oret_ret_sn= 'N' WHERE id_causado= " + regRetDet.getId_causado());
