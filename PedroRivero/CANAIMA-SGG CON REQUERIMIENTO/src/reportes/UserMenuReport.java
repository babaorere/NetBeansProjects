...
public final class UserMenuReport extends XJFrameJTable {
     ...
     * @param _parent
     */
    public UserMenuReport(final java.awt.Window _parent) {
        super(_parent);
        initComponents();
    ...
    private void initComponents() {
        ...
        tblMaster.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "ID", "User", "Menú", "Oper.", "Observación", "Acceso User", "Acceso Serv."
            }
        ) {
    ...
    private void btnReportSummaryActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnReportSummaryActionPerformed
        ...
        try {
            ...
            Conn.getConnection().prepareStatement("DELETE FROM user_menu_track_summary WHERE fchsession <= '" + fchDelete + "' OR iduser=" + UserPassIn.getIdUser() + " AND idsession=" + UserPassIn.getIdSession()).executeUpdate();
        } catch (final Exception _ex) {
            JOptionPane.showMessageDialog(null, _ex);
            return;
        }
        ...
        for (int i = 0; i < tblMaster.getRowCount(); i++) {
            ...
            try {
                final PreparedStatement pst = Conn.getConnection().prepareStatement("INSERT INTO user_menu_track_summary(user,mnuclass,op,obs,userdatetime,servdatetime,fchsession,idsession,iduser) VALUES (?,?,?,?,?,?,?,?,?)");
