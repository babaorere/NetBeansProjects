...
public final class CreditoAdicionalReport extends XJFrameJTable {
     ...
     * @param _parent
     */
    public CreditoAdicionalReport(final java.awt.Window _parent) {
        super(_parent);
        initComponents();
    ...
    private void initComponents() {
        ...
        tblMaster.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Soporte", "Descripción", "Monto Bs.", "Fecha Cred.", "Fecha Gen."
            }
        ) {
        ...
        lblmonto1.setFont(new java.awt.Font("Tahoma", 1, 16)); // NOI18N
    ...
    private void btnReportSummaryActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnReportSummaryActionPerformed
        ...
        try {
            Conn.getConnection().prepareStatement("DELETE FROM creditoadicional_rpt_summary WHERE fchsession <= '" + fchDelete + "' OR iduser=" + UserPassIn.getIdUser() + " AND idsession=" + UserPassIn.getIdSession()).executeUpdate();
        } catch (final Exception _ex) {
            JOptionPane.showMessageDialog(null, _ex);
            return;
        }
        ...
        for (int i = 0; i < tblMaster.getRowCount(); i++) {
            ...
            try {
                final PreparedStatement pst = Conn.getConnection().prepareStatement("INSERT INTO creditoadicional_rpt_summary(soporte,descripcion,monto,fecha,fchsession,idsession,iduser) VALUES (?,?,?,?,?,?,?)");
    ...
    private void CreditoAdicionalRpt(int _selRow) throws Exception {
        creAdi = (TblCreditoAdicional) tblMaster.getValueAt(_selRow, 0);
