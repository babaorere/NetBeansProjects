...
public class PagoModel implements PagAvaInterface {
    ...
    public static boolean isByCheck(final long inid) throws Exception {
        ...
        try (final PreparedStatement pstPago = ConnCapip.getInstance().getConnection().prepareStatement("SELECT * FROM orden_pago "
                + "WHERE id_orden_pago= ?")) {
            ...
            try (final ResultSet rsPago = pstPago.executeQuery()) {
                ...
                if (rsPago.next()) {
                    ...
                    try (final PreparedStatement pstBanco = ConnCapip.getInstance().getConnection().prepareStatement("SELECT * FROM bancos_operaciones "
