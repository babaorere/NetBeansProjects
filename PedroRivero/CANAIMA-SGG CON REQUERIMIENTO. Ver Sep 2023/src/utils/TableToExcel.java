...
public class TableToExcel {
    ...
    public static void exportExcel(final JTable intable, final String infileName) throws Exception {
        ...
        File file = new File(CapipPropiedades.CAPIP_DIR_LOCAL + "/" + infileName);

        HSSFWorkbook fWorkbook = new HSSFWorkbook();
        HSSFSheet fSheet = fWorkbook.createSheet("Reporte");

        HSSFRow headerRow = fSheet.createRow(0); //Create row at line 0
        for (int headings = 0; headings < intable.getColumnCount(); headings++) { //For each column
    ...
    public static void exportTSV(JTable intable, final String infileName) throws IOException {
        ...
        FileWriter out = new FileWriter(new File(CapipPropiedades.CAPIP_DIR_LOCAL + "/" + infileName));
